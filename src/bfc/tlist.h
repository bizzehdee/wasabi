...
public:
  ...
  int delByPos(int pos) {
    ...
    MEMMOVE(items+pos, items+(pos+1), sizeof(T)*(nitems-pos));  // CT:not (nitems-(pos+1)) as nitems has been decremented earlier
    return 1;
  }
  ...
  int removeItem(const T &it) {	// note: removes only first found
    int pos = getItemPos(it);

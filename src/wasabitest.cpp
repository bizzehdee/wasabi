...
public:
  XmlTest() : m_failed(0), m_inroot(0), m_intest1(0), m_intest2(0) {
    ...
    str += "</WasabiTest>\n";
    XmlReader::registerCallback("*", static_cast<XmlReaderCallback*>(this));
    XmlReader::loadFile(str, NULL, static_cast<XmlReaderCallback*>(this));
    XmlReader::unregisterCallback(static_cast<XmlReaderCallback*>(this));
  }
  virtual ~XmlTest() {
  }
...
int main(int argc, char **argv) {
  ...
  ApiInit::init(INVALIDOSMODULEHANDLE, myappguid, "", INVALIDOSWINDOWHANDLE);
  ...
  for (i=0;i<n;i++) {
    ...
    printf("  %d : %s (%s)\n", i, factory->getServiceName(), Guid(factory->getGuid()).toChar().vs());
  }
  if (i > 0) {
  printf("\n- PASSED -\n\n");
  } else {

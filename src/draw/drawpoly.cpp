...
#if 0//CUT
static void premultiply(ARGB32 *m_pBits, int nwords) {	//CUT
...
void Draw::drawPointList(ARGB32 *bits, int w, int h, const char *pointlist) {
  if (pointlist == NULL || *pointlist == '\0') return;
  PathParser outer(pointlist, "|");
  const char *pl;
  for (int i = 0; (pl = outer.enumString(i)) != NULL; i++) {
    PathParser inner(pl, "=");
#ifdef WASABI_COMPILE_SKIN
    ARGB32 color = WASABI_API_SKIN->parse(inner.enumStringSafe(1, "255,255,255,255"), "coloralpha");
#else
    ARGB32 color = 0xffff00ff;	//FUCKO
#endif
    int a = color & 0xff000000;
    color = BSWAP(color<<8) | a;
    StdMath::premultiplyARGB32(&color);
    beginPolygon(bits, w, h, color);
    PathParser eener(inner.enumStringSafe(0, "0,0"), ";");
    const char *cc;
    for (int j = 0; (cc = eener.enumString(j)) != NULL; j++) {
      PathParser com(cc, ",");
      const char *xs = com.enumStringSafe(0, "0");
      int x = STRCHR(xs, '.') ? (int)floor(ATOF(xs) * w + .5f) : ATOI(xs);
      const char *ys = com.enumStringSafe(1, "0");
      int y = STRCHR(ys, '.') ? (int)floor(ATOF(ys) * h + .5f) : ATOI(ys);
      addPoint(x, y);
    }
    endPolygon();
  }
...
void Draw::rect(ARGB32 *bits, int cw, int ch, int x, int y, int w, int h, ARGB32 color, int filled, int alpha, int apply_color_alpha) {
  alpha = MINMAX(alpha, 0, 255);
  if (apply_color_alpha) StdMath::premultiplyARGB32(&color);
  // unoptimized yet succint
  for (int py = 0; py < h; py++) {
    int zy = py + y;
    if (zy < 0 || zy >= ch) continue;
    for (int px = 0; px < w; px++) {
      int zx = px + x;
      if (zx < 0) continue;
      if (zx >= cw) break;
      if (filled || !px || !py || px == w - 1 || py == h - 1) {
        ARGB32 *pt = bits + cw * zy + zx;
        if (apply_color_alpha)
          *pt = Blenders::BLEND_ADJ3(*pt, color, alpha);
        else
          *pt = Blenders::BLEND_ADJ1(*pt, color, alpha);
      }
    }
  }
}

void Draw::hline(ARGB32 *bits, int cw, int ch, int x1, int y1, int x2, ARGB32 color, int alpha, int apply_color_alpha) {
  alpha = MINMAX(alpha, 0, 255);
  ...
  for (int px = 0; px < w; px++) {
    ...
    if (zx < 0) continue;
    if (zx >= cw) break;
    ARGB32 *pt = bits + cw * y1 + zx;

    if (apply_color_alpha)
      *pt = Blenders::BLEND_ADJ3(color, *pt, alpha);
    else
      *pt = Blenders::BLEND_ADJ1(color, *pt, alpha);
  }
}

...
#define XNFNAME "wasabi.xmlconfig"
...
void Reader::xmlReaderOnStartElementCallback(const char *xmlpath, const char *xmltag, XmlReaderParams *params) {
  ...
  if (!STRICMP(xmltag, "entry")) {
    ...
    // just in case
    if (STRCASEEQL(name, "(null)")) name = NULL;
    if (STRCASEEQL(value, "(null)")) value = NULL;
...
//CUT  XmlReader::registerCallback("WinampXML/configuration", this);
//CUT  XmlReader::registerCallback("WinampXML/configuration/*", this);
...
String ConfigFile::makeName(const char *name) {
  String ret = name;
  if (!sectionname.isempty()) {
    ret.prepend("/");
    ret.prepend(sectionname);
  }
  return ret;
}

StringPair *ConfigFile::getPair(const char *name) {
  return strings.findItem(makeName(name));
}

StringPair *ConfigFile::makePair(const char *name, const char *value) {
  StringPair *ret = getPair(name);
  if (ret == NULL) {
    ret = new StringPair(makeName(name), value);
    strings.addItem(ret);
  } else {
...
ConfigFile::ConfigFile(const char *section, const char *name) {
  sectionname = section;
  prettyname = name;
  ninstances++;
}
...
ConfigFile::~ConfigFile() {
  ...
  if (ninstances == 0) {
    ...
    if (fp != NULL) {
      ...
      const char *app = WASABI_API_APP->main_getVersionString();
      if (app != NULL)
        xml.comment(StringPrintf("Generated by: %s (%d) on %s", app, WASABI_API_APP->main_getBuildNumber(), StringStrftime().vs()));
      ...
      for (int i = 0; i < strings.getNumItems(); i++) {
        ...
        enc = p->b;
        Url::encode(enc, FALSE, URLENCODE_EXCLUDE_7BIT_ABOVEEQ32);
        if (enc.iscaseequal("(null)") || enc.getValue() == NULL)
        enc.setValue("");
...
void ConfigFile::setInt(const char *name, int val) {
...
int ConfigFile::getInt(const char *name, int def_val) {
  INCRITICALSECTION(cs);
...
void ConfigFile::setString(const char *name, const char *str) {
  INCRITICALSECTION(cs);
  if (name == NULL) return;
  if (!verifyName(name)) {
    DebugString("illegal name given\n");
...
int ConfigFile::getString(const char *name, char *buf, int buf_len, const char *def_str) {
  INCRITICALSECTION(cs);
  if (name == NULL || buf == NULL) return -1;
  if (def_str == NULL) def_str = "";
  StringPair *p = getPair(name);

  // strncpyTo is wayyy safer than STRNCPY
  if (p == NULL)
    String(def_str).strncpyTo(buf, buf_len);
  else
    p->b.strncpyTo(buf, buf_len);
  return 1;
}

int ConfigFile::getStringLen(const char *name) {
  INCRITICALSECTION(cs);

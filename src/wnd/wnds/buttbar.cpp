...
//#include "api/script/c_script/h_button.h"
/*
...
void ButtBar::rootwndholder_getRect(RECT *r) {
  //BUTTBAR_PARENT::rootwndholder_getRect(r);  
}
...
int ButtBar::addChild(ButtonWnd *child) {
  ...
  if (isInited()) {
    ...
    onResize();
    if (buttons.getNumItems() == 1) setGroupLabel(child->getButtonText());
  }
  return 1;
}
...
int ButtBar::getHeight() {
  if (resize_mode == STACK) {
    //RootWnd *rw = getContentRootWnd();
    //return !rw ? getPreferences(SUGGESTED_H) : rw->getPreferences(SUGGESTED_H);
    return 10;
  } else {
    int h = 0;
    for (int i = 0; i < buttons.getNumItems(); i++) {
...
void ButtBar::setGroupLabel(const char *l) {
  setName(l);
  onNewContent();
}
...
void ButtBar::onNewContent() {/*
  ...
  ScriptObject *text = findScriptObject("buttonbar.text");
  if (text == NULL) return;
  C_Text(text).setText(getNameSafe("no tabs"));

...
class COMEXP Accessible : public Dispatchable {
  public:
    ...
    void onSetName(const char *newname, int idx=-1);
...
inline void Accessible::onSetName(const char *name, int idx) {
  _voidcall(ACCESSIBLE_ONSETNAME, name, idx);
}

inline OSWINDOWHANDLE Accessible::getOSWnd() {
  return _call(ACCESSIBLE_GETOSWND, INVALIDOSWINDOWHANDLE);
}
...
class AccessibleI : public Accessible {
  public:
    ...
    virtual void onSetName(const char *name, int idx)=0;
